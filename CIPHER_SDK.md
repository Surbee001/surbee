# Cipher SDK for Developers

**Date:** January 2025
**Purpose:** Developer SDK for integrating Cipher fraud detection into third-party applications

---

## Executive Summary

The Cipher SDK allows developers to integrate our 7-layer fraud detection system into their own survey platforms, forms, and data collection systems using simple API calls. Developers use their own Surbee API keys, and Cipher runs on our infrastructure.
...
sentinel = SentinelClient(api_key=os.getenv('Cipher_API_KEY'))
...
export async function handleTypeformWebhook(req, res) {
  ...
  // Convert Typeform format to Cipher format
  const responses = {}
...
function SurveyForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Send to your backend, which calls Cipher
...
Run Cipher analysis asynchronously after accepting the submission:
...
The Cipher SDK client libraries are open-source (MIT License).
The Cipher API service requires a valid API key and subscription.
