...
const styles = `  *:focus-visible {    outline-offset: 0 !important;    --ring-offset: 0 !important;  }  textarea::-webkit-scrollbar {    width: 6px;  }  textarea::-webkit-scrollbar-track {    background: transparent;  }  textarea::-webkit-scrollbar-thumb {    background-color: #444444;    border-radius: 3px;  }  textarea::-webkit-scrollbar-thumb:hover {    background-color: #555555;  }
`;
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              'rounded-3xl border border-[#444444] bg-[#1f2121] p-2 shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300',
...
export const PromptInputBox = React.forwardRef(
  (props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const [isEditSelected, setIsEditSelected] = React.useState(false);
    ...
    const handleSubmit = () => {
      if (input.trim() || files.length > 0) {
        let messagePrefix = '';
        if (showThink) messagePrefix = '[Brainstorm: ';
    ...
    return (
      <>
        <PromptInput
          ...
          className={cn(
            'w-full bg-[#1f2121] border-[#444444] shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300 ease-in-out',
        ...
        >
          ...
          >
            <PromptInputTextarea
              placeholder={showThink ? 'Brainstorm ideas...' : placeholder}
              className="text-base"
          ...
          <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
            <div
              className={cn(
                'flex items-center gap-1 transition-opacity duration-300 ml-1',
            ...
            >
              <Button
                ...
                size="icon"
                className={cn(
                  'h-6 w-6 rounded-full border border-zinc-600 transition-all duration-200',
                  isRecording
              ...
              >
                {isLoading ? (
                  <Square className="h-3 w-3 fill-[#1F2023] animate-pulse" />
                ) : isRecording ? (
                  <StopCircle className="h-3 w-3 text-red-500" />
                ) : hasContent ? (
                  <ArrowUp className="h-3 w-3 text-[#1F2023]" />
                ) : (
                  <Mic className="h-3 w-3 text-[#1F2023] transition-colors" />
                )}
              </Button>
            </PromptInputAction>
          </PromptInputActions>
