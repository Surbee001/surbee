# Cipher Sandbox Integration Design

**Date:** January 2025
**Purpose:** Design document for embedding Cipher fraud detection into the sandbox for 24/7 automatic operation

---

## Executive Summary

Cipher will be embedded into the survey sandbox as an invisible, always-on fraud detection layer that automatically analyzes all survey responses in real-time. The system will be completely transparent to survey respondents while providing comprehensive fraud analysis to survey administrators.
...
Cipher will be implemented as **server-side middleware** that intercepts all survey submissions before they are stored in the database.

```typescript
// src/app/api/surveys/[id]/submit/route.ts

import { NextRequest, NextResponse } from 'next/server'
import { runCipherAnalysis } from '@/lib/sentinel/sentinel-api'
...
export async function POST(
  ...
  try {
    ...
    // 1. Get survey-specific Cipher configuration
    const surveyConfig = await getSurveyConfig(surveyId)

    // 2. Run Cipher fraud detection (invisible to user)
    const fraudAnalysis = await runCipherAnalysis({
      surveyId,
      responses,
      behavioralMetrics,
      deviceInfo,
...
/**
 * Main Cipher API - runs all fraud detection layers
 */
export async function runCipherAnalysis(
  input: CipherInput
): Promise<CipherOutput> {
  ...
  try {
    ...
    // Return safe default (don't block submission on Cipher failure)
    return {
      fraudScore: 0,
      riskLevel: 'low',
      isLikelyFraud: false,
...
Cipher_ENABLED=true
Cipher_MODE=production # 'development' | 'production'
Cipher_ASYNC=true # Process in background to avoid blocking submissions
...
# Detection Settings
Cipher_DEFAULT_SENSITIVITY=balanced # 'strict' | 'balanced' | 'lenient'
Cipher_AUTO_REJECT=false # Don't auto-reject by default
Cipher_NOTIFY_ADMIN=true # Email admins on fraud detection

# Performance
Cipher_TIMEOUT_MS=5000 # Max time for analysis
Cipher_CACHE_RESULTS=true # Cache fraud ring analysis
```
...

{
  "functions": {
    "src/app/api/surveys/[id]/submit/route.ts": {
      "maxDuration": 10, // Allow up to 10s for Cipher processing
...
```typescript
// Process Cipher analysis in background
export async function POST(req: NextRequest) {
  ...
  // Run Cipher in background (don't block response)
  runCipherAnalysisAsync(savedResponse.id, submission)

  // Return success immediately
  return NextResponse.json({ success: true })
}

async function runCipherAnalysisAsync(
  responseId: string,
  submission: any
) {
  try {
    const fraudAnalysis = await runCipherAnalysis(submission)
    ...
    console.error('Async Cipher error:', error)
  }
}
```
...
describe('Cipher Sandbox Integration', () => {
  ...
  it('should not block submission on Cipher failure', async () => {
    // Mock Cipher failure
    jest.spyOn(sentinelApi, 'runCipherAnalysis').mockRejectedValue(
...
```typescript
// Monitor Cipher processing time
import * as Sentry from '@sentry/nextjs'

export async function runCipherAnalysis(input: CipherInput) {
  const transaction = Sentry.startTransaction({
    op: 'sentinel.analysis',
    name: 'Cipher Fraud Detection',
  })
...
alerts:
  ...
  - name: Cipher Processing Slow
    condition: avg_processing_time_ms > 5000
    action: slack_notification
    message: "Cipher taking >5s per analysis"

  - name: Cipher Failures
    condition: error_rate > 0.05
    action: pagerduty
    message: "Cipher failing on >5% of submissions"
```
...
Cipher will be seamlessly integrated into the survey sandbox as an always-on, invisible fraud detection layer. The system will:
