...
// ThemeEditor is now inside ProjectSettings as a tab
...
export default function ProjectPage() {
  ...
  // Theme editor opens project settings on the theme tab
  ...
  const createNewBlockSurvey = useBlockEditorStore(s => s.createNewSurvey);
  ...
  // Reset block editor when navigating to a different project
  useEffect(() => {
    if (projectId && blockEditorSurvey && blockEditorSurvey.id !== projectId) {
      createNewBlockSurvey(projectId, 'Untitled Survey');
    }
      ...
      fetch(`/api/projects/${projectId}?userId=${user.id}`)
              ...
              return fetch(`/api/projects/${projectId}`, {
                ...
                .then((createData) => {
                  if (createData?.project) {
                    ...
                    // Initialize a blank block survey for new projects
                    if (!blockEditorSurvey) {
                      createNewBlockSurvey(projectId, 'Untitled Survey');
                    }
                    return createData;
                  }
        ...
        .then((data) => {
          if (data?.project) {
            ...
            // If project has no block_survey and no sandbox, initialize a blank survey
            if (!data.project.block_survey && !data.project.sandbox_bundle && !blockEditorSurvey) {
              createNewBlockSurvey(projectId, 'Untitled Survey');
            }
          }
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.publishedUrl) {
        ...
        const { updateProject: updateStoreProject, addProject: addStoreProject, projects: cachedProjects } = useUserStore.getState();
        if (cachedProjects.some(p => p.id === projectId)) {
          ...
          // Project not in cache yet — add it so the projects list shows it as published
          addStoreProject({
            id: projectId,
            title: autoGeneratedTitle || updatedProject.title || 'Untitled Survey',
            status: 'published',
            updated_at: new Date().toISOString(),
            published_url: data.publishedUrl,
            preview_image: null,
    ...
    <AppLayout hideSidebar fullBleed>
          ...
          >
            {sidebarView === 'history' ? (
              ...
              <div className="flex-1 overflow-y-auto pl-4 pr-2 pt-6 thin-scrollbar" style={{ paddingBottom: '80px' }} ref={chatAreaRef}>
                {/* Quick suggestion pills — centered in chat when empty */}
                {!hasStartedChat && messages.length === 0 && status === 'ready' && (
                  <div style={{
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    ...
                    <div style={{
                      ...
                      {[
                        ...
                        <button
                          key={i}
                          onClick={() => handleSubmit(`Create a ${suggestion.toLowerCase()}`)}
                          style={{
                            padding: '8px 14px',
                            fontSize: '13px',
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden" style={{ backgroundColor: 'var(--surbee-sidebar-bg)' }}>
        ...
        <div className="flex items-center justify-between pl-2 pr-3 pt-3 pb-0" style={{ backgroundColor: 'var(--surbee-sidebar-bg)' }}>
          ...
          {/* Right Section - Theme, Preview, Upgrade & Publish Buttons */}
          <div className="relative flex items-center gap-2">
            ...
            {editorMode === 'blocks' && blockEditorAvailable && (
              <Tooltip>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: isPreviewMode
                        ...
                        : 'transparent',
                      color: isDarkMode ? '#ffffff' : '#000000',
                      opacity: (!blockEditorHasContent && !isPreviewMode) ? 0.3 : 1,
                    }}
                    onMouseEnter={(e) => {
                      if (!blockEditorHasContent && !isPreviewMode) return;
                      const isDark = document.documentElement.classList.contains('dark');
                      e.currentTarget.style.backgroundColor = isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)';
                    }}
                    onMouseLeave={(e) => {
                    ...
                    disabled={!blockEditorHasContent && !isPreviewMode}
                  >
                    {isPreviewMode ? (
                ...
                <TooltipContent side="bottom" sideOffset={4}>
                  {isPreviewMode ? 'Back to editor' : (!blockEditorHasContent ? 'Add blocks to preview' : 'Preview')}
                </TooltipContent>
              </Tooltip>
            )}
            <button
                      ...
                      >
                        {(status === 'submitted' || status === 'streaming') && !sandboxAvailable ? (
                          ...
                          <div className="flex items-center justify-center h-full">
                            <div style={{ width: '100%', maxWidth: '400px', padding: '0 32px', opacity: 0.1 }}>
                              <div style={{ height: 24, background: 'var(--surbee-fg-muted)', borderRadius: 6, width: '75%', marginBottom: 16 }} />
                              <div style={{ height: 16, background: 'var(--surbee-fg-muted)', borderRadius: 4, width: '100%', marginBottom: 8 }} />
                              <div style={{ height: 16, background: 'var(--surbee-fg-muted)', borderRadius: 4, width: '85%', marginBottom: 24 }} />
                              <div style={{ height: 40, background: 'var(--surbee-fg-muted)', borderRadius: 8, width: '100%', marginBottom: 10 }} />
                              <div style={{ height: 40, background: 'var(--surbee-fg-muted)', borderRadius: 8, width: '100%', marginBottom: 10 }} />
                              <div style={{ height: 40, background: 'var(--surbee-fg-muted)', borderRadius: 8, width: '100%' }} />
                            </div>
                          </div>
                        )}
                      </div>
