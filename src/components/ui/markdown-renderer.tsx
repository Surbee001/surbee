...
// Module-scope inline markdown parser (no component deps)
...
// Module-scope markdown parser (no component deps)
const parseMarkdown = (text: string): React.ReactNode[] => {
  ...
  const flushParagraph = () => {
    if (currentParagraph.length > 0) {
      ...
      elements.push(
        <p key={elements.length} className="mb-4 text-[16px] leading-7" style={{ color: 'var(--surbee-fg-primary)' }}>
      ...
      elements.push(
        <ul key={elements.length} className="mb-4 ml-4 space-y-1">
          {listItems.map((item, i) => (
            <li key={i} className="text-[16px] leading-7 list-disc" style={{ color: 'var(--surbee-fg-primary)' }}>
      ...
      if (match) {
        elements.push(
          <ol key={elements.length} className="mb-4 ml-4 space-y-1" start={parseInt(match[1])}>
            <li className="text-[16px] leading-7 list-decimal" style={{ color: 'var(--surbee-fg-primary)' }}>
