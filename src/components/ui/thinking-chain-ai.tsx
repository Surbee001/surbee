...
function syntaxHighlight(html: string): string {
  ...
  highlighted = highlighted.replace(
    /(&lt;\/?)(\w+)([^&]*?)(&gt;)/g,
    '$1<span style="color: #fc9ce0">$2</span><span style="color: #a78bfa">$3</span>$4'
  )

  // HTML attributes
  highlighted = highlighted.replace(
    /\b(\w+)=(&quot;[^&]*?&quot;|&#039;[^&]*?&#039;)/g,
    '<span style="color: #5ccfe6">$1</span>=<span style="color: #fcb34a">$2</span>'
  )

  // CSS within style tags or attributes
  highlighted = highlighted.replace(
    /(style=&quot;[^&]*?)([a-zA-Z-]+)(\s*:\s*)([^;&quot;]+)/g,
    '$1<span style="color: #5ccfe6">$2</span>$3<span style="color: #fcb34a">$4</span>'
  )

  // JavaScript/CSS keywords
  highlighted = highlighted.replace(
    /\b(function|const|let|var|if|else|for|while|return|import|export|default|class|extends|async|await|try|catch|finally)\b/g,
  ...
  highlighted = highlighted.replace(
    /([.#][\w-]+|\b[\w-]+)(\s*:\s*)([^;}&quot;]+)(;?)/g,
    '<span style="color: #5ccfe6">$1</span>$2<span style="color: #fcb34a">$3</span><span style="color: #6b7280">$4</span>'
  )
...
function CodeContainer({ htmlContent, isStreaming }: { htmlContent: string; isStreaming: boolean }) {
  ...
  // Simple syntax highlighting for HTML tags only
  const highlightedCode = useMemo(() => {
    if (!debouncedContent) return ''
    // Only highlight HTML tags, not attributes to avoid corruption
    return debouncedContent
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/&lt;(\/?[a-zA-Z][a-zA-Z0-9]*[^&]*?)&gt;/g, '<span style="color: #fc9ce0">&lt;$1&gt;</span>')
  }, [debouncedContent])
