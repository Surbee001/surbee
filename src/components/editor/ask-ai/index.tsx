...
export function AskAI({
  ...
  const buildPartialDoc = (raw: string) => {
    ...
    let fragment = '';
    if (doctypeIdx !== -1) fragment = raw.slice(doctypeIdx);
    else if (htmlIdx !== -1) fragment = raw.slice(htmlIdx);
    else if (bodyIdx !== -1) fragment = raw.slice(bodyIdx);
    else return '';
    ...
    if (/\<head[\s>]/i.test(doc) && !/\<\/head>/i.test(doc)) doc += '\n</head>';
    if (/\<body[\s>]/i.test(doc) && !/\<\/body>/i.test(doc)) doc += '\n</body>';
    if (!/\<\/html>/i.test(doc)) doc += '\n</html>';
    return doc;
  };
  ...
  const callAi = async (redesignMarkdown?: string) => {
    ...
    try {
      // Fast classifier gate (skip for pure redesign flow)
      if (!redesignMarkdown && prompt.trim()) {
        try {
          const clsRes = await fetch('/api/surbee/classify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ prompt }),
          })
          const cls = await clsRes.json()
          if (!clsRes.ok || cls.isSurveyLike === false) {
            setisAiWorking(false)
            toast.warning(
              cls?.reason ||
                'This looks like a general website request. Surbee focuses on surveys/forms/quizzes. Please rephrase your request as a survey.'
            )
            return
          }
        } catch {
          // If classifier fails, continue (do not block)
        }
      }
      ...
      if (isFollowUp && !redesignMarkdown && !isSameHtml) {
        ...
        if (request && request.body) {
          ...
          const contentThink: string | undefined = undefined;
          const read = async () => {
            ...
            if (done) {
              ...
              const finalDoc = contentResponse.match(/<!DOCTYPE html>[\s\S]*<\/html>/i)?.[0];
            ...
            let newHtml = contentResponse.match(/<!DOCTYPE html>[\s\S]*/i)?.[0] || '';
            if (!newHtml) newHtml = buildPartialDoc(contentResponse);
            if (newHtml) {
              setIsThinking(false);
              const now = Date.now();
              ...
              if (newHtml.length > 200) onScrollToBottom();
            }
            read();
          };
          read();
        ...
        if (request && request.body) {
          ...
          const buildPartialDoc = (raw: string) => {
            // Try to locate a sensible starting point
            const doctypeIdx = raw.indexOf('<!DOCTYPE html>');
            const htmlIdx = raw.toLowerCase().indexOf('<html');
            const bodyIdx = raw.toLowerCase().indexOf('<body');
            let fragment = '';
            ...
            if (!/\<head[\s>]/i.test(doc)) {
              doc = doc.replace(/<html[^>]*>/i, (m) => `${m}\n<head></head>`);
              if (!/\<html[^>]*>/i.test(doc)) {
                // If body fragment only
                doc = `<!DOCTYPE html>\n<html>\n<head></head>\n${doc}`;
              }
            }
            // Ensure closings for head/body/html so iframe renders
            if (/\<head[\s>]/i.test(doc) && !/\<\/head>/i.test(doc)) doc += '\n</head>';
            if (/\<body[\s>]/i.test(doc) && !/\<\/body>/i.test(doc)) doc += '\n</body>';
            if (!/\<\/html>/i.test(doc)) doc += '\n</html>';
            return doc;
          ...
          const read = async () => {
            ...
            let newHtml = contentResponse.match(/<!DOCTYPE html>[\s\S]*/i)?.[0] || '';
            if (!newHtml) {
              newHtml = buildPartialDoc(contentResponse);
            }
            if (newHtml) {
              setIsThinking(false);
              const partialDoc = newHtml;
              // buildPartialDoc already ensures proper closures
