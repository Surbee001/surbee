...
import type { RadioContent, Option } from '@/lib/block-editor/types'

export const RadioBlock: React.FC<BlockComponentProps<'radio'>> = ({
  block,
  isEditing,
  onContentChange,
  onFocus,
}) => {
  const content = block.content as RadioContent

  const addOption = () => {
    const newOption: Option = {
      id: nanoid(8),
      label: `Option ${content.options.length + 1}`,
      value: `option_${content.options.length + 1}`,
    }
    ...
    <div onClick={onFocus} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      {(content.description || isEditing) && isEditing ? (
        <input
          ...
          onChange={(e) => onContentChange({ description: e.target.value || undefined })}
        ...
        <p style={{ fontSize: '0.825rem', color: 'var(--surbee-fg-muted)', margin: 0 }}>{content.description}</p>
      ) : null}
      ...
      {isEditing && (
        <button
          ...
          style={{
            background: 'transparent',
            border: 'none',
            color: 'var(--surbee-accent-primary, #2563eb)',
            cursor: 'pointer',
            fontSize: '0.85rem',
            padding: '4px 0',
            textAlign: 'left',
