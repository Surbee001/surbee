...
import type { TextInputContent } from '@/lib/block-editor/types'

export const TextInputBlock: React.FC<BlockComponentProps<'text-input'>> = ({
  block,
  isEditing,
  isFocused,
  onContentChange,
  ...
  const content = block.content as TextInputContent
  ...
  return (
    <div onClick={onFocus} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
        ...
        {content.required && (
          <span style={{ color: 'var(--surbee-semantic-error, #ef4444)', fontSize: '0.875rem' }}>*</span>
        )}
      </div>
      ...
      {(content.description || isEditing) && (
        isEditing ? (
          ...
          <p style={{ fontSize: '0.825rem', color: 'var(--surbee-fg-muted)', margin: 0 }}>
            {content.description}
          </p>
        ) : null
      )}
